...
correlations_data <- nla_data_subset |>
  ...
  # filter(year == "2012") |> # same as total except TP better predictor in xeric and w. mtn
...
write.csv(criteria, "testfolder/criteria07_12.csv")
...
## find only sites that are sampled in both 2007 and 2012
...
ecoreg_plot <- ggplot(lim_changes_fullset |>
  ...
  # ecoregions from 2007-2012. The change is represented as a percent difference in the population (point) with 
...
ggplot(TS_changes_fullset) +
  ...
  labs(x = "",
       y = "% change 2007-2012")+                                                 
  #        caption = "Figure 6. Percent change in trophic state across lakes nationally from 2007-2012, panels separated to show all lakes 
