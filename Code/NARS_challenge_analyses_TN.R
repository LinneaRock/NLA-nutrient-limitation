...
# prep the data
limits_change_prep_TN <- limitsTN|> # total 2033 lakes for this analysis 
...
nrow(limits_change_prep_TN |> filter(limitation == "P-limitation")) # 679 
nrow(limits_change_prep_TN |> filter(limitation == "N-limitation")) # 1020
nrow(limits_change_prep_TN |> filter(limitation == "Co-nutrient limitation")) # 334

years <- c("2007", "2017")
percent_lim <- data.frame()

for(i in 1:length(years)) {
  data <- limits_change_prep_TN |>
    filter(year == years[i])
