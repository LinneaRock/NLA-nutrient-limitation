...
### How have the lakes changed trophic category (according to Chlorophyll status) over the years?
Looking at the difference between 2017 and 2007

```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=6.5, fig.height=4.5}

chla_change_ecoreg <- change_analysis(ts |> filter(year %in% c("2007", "2017")), siteID = "UNIQUE_ID", subpop = "ECO_REG_NAME", vars_cat = "TSTATE_CHL", surveyID = "year", weight = "WGT_NLA", xcoord = "LON_DD", ycoord = "LAT_DD")

chla_change_ecoreg.1 <- chla_change_ecoreg[["catsum"]]  |>
  select(Subpopulation, Category, Indicator, DiffEst.P, StdError.P)


chla_change_national <- change_analysis(ts |> filter(year %in% c("2007", "2017")), siteID = "UNIQUE_ID", vars_cat = "TSTATE_CHL", surveyID = "year", weight = "WGT_NLA", xcoord = "LON_DD", ycoord = "LAT_DD")

chla_change_national.1 <- chla_change_national[["catsum"]] |>
  select(Subpopulation, Category, Indicator, DiffEst.P, StdError.P) |>
  mutate(Subpopulation = "National") 


chla_total_change0717 <- rbind(chla_change_national.1, chla_change_ecoreg.1) 

#nat.plot.chla <-
ggplot(chla_total_change0717 |> filter(Subpopulation == "National")) +
  ...
  labs(x = "",
       y = "% Difference 2007, 2017 Lakes") +
  scale_x_discrete(limits = rev(levels(as.factor(chla_total_change0717$Category)))) 
