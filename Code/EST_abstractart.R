...
ggplot() +
  ...
  geom_sf(data = nla_locations.sf |> filter(year == '2017', limitation %in% c("N-limitation","P-limitation")), aes(color = limitation, shape=limitation), size = 0.5) +
  geom_sf(data = nla_locations.sf |> filter(year == '2017', limitation %in% c("Co-limitation")), aes(color = limitation),shape=16, size = 0.5) +
  theme_minimal() +
  scale_fill_manual("", values = muted) +
  scale_color_manual("", values = c("grey60","red4", "#336a98"), labels = c('Co-limitation', 'N-limitation', 'P-limitation'))  +
